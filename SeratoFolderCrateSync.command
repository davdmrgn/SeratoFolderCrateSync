...
class Database:
  ...
  class Temp:
    def Create():
      ...
      logging.debug(f'Create temporary database at {self}')
    ...
    def Remove(self):
      if os.path.exists(self):
        logging.info(f'Removing temporary database: {self}')
        shutil.rmtree(self)
  ...
  def Backup():
    ...
    print()
    logging.info(f'Backing up database at {database_folder} to {backup_folder_now}')
    copy_ignore = shutil.ignore_patterns('.git*', 'Recording*', 'DJ.INFO')
    shutil.copytree(database_folder, backup_folder_now, ignore=copy_ignore, symlinks=True)
    logging.info(f'\033[1;92mBackup done!\033[0m')

  def Restore():
    backup_folder = database_folder + 'Backups'
    if not os.path.exists(backup_folder):
      logging.error(f'\033[1;93mBackup folder not found\033[0m')
      time.sleep(1)
    else:
      backups = []
      ...
      if len(backups) == 0:
        logging.error(f'\033[1;93mNo backups found\033[0m')
        time.sleep(1)
      else:
        logging.info('\nRestore from backup\n')
        ...
        if os.path.exists(restore_folder):
          ...
          if re.match('y|yes', answer):
            try:
              ...
              logging.info(f'\033[1;92mRestore done!\033[0m')
            except:
              logging.error(f'\033[1;93mError restoring database\033[0m')
          else:
            logging.warning(f'\n\033[1;93mNOPE\033[0m')
  ...
  def Apply(temp_database):
    ...
    if re.match('y|yes', menu.lower()):
      ...
      logging.info(f'\033[1;33mNot applying changes!\033[0m')
      return False
...
class SeratoData:
  def Read(self):
    logging.info(f'Reading Serato database file: {self}')
    ...
    for item in self:
      ...
      if key == 'otrk':
        ...
        for item in otrk_item:
          if re.match('pfil|ptrk', item[0]):
            p_value = re.sub(find, replace, item[1])
            i += 1
            terminal_width = os.get_terminal_size().columns - 20
    ...
    for line in self:
      if line[0] == 'otrk':
        ...
        if os.path.exists(file_path):
          ...
          logging.warning(f'\033[1;93mMISSING!\033[0m {file_path}\033[K')
          database_music.append(file_path)
...
class Crate:
  def Sync(rebuild = False):
    ...
    if crate_check > 0:
      logging.info(f'{crate_check} crate{"s" if crate_check > 1 else None} to update\033[K')
      apply = Database.Apply(temp_database)
      if apply == True:
        logging.info(f'\033[1;32mSync done!\033[0m')
    elif crate_check == 0:
      logging.info(f'\033[K\r\033[1;32mNo crate updates required\033[0m')
    time.sleep(1)
    ...
    for root, dirs, files in os.walk(temp_database):
      for crate_file in files:
        if crate_file.endswith('crate'):
          ...
          logging.info(f'Scanning {"Smart" if crate_file.endswith(".scrate") else None}Crate: {crate_fullpath_displayname}')
          ...
          logging.debug('Writing updates: ' + crate_fullpath)
